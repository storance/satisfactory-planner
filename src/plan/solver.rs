use crate::game::{Recipe, Resource, ResourceDefinition, ResourceValuePair};
...
impl<'a, 'b> Solver<'a, 'b> {
    ...
    fn solve_for_item(
        &self,
        resource_value: ResourceValuePair<f64>,
        ...
        if resource_value.resource.is_raw() {
            let input_limit = *self
                .config
                .input_limits
                .get(&resource_value.resource)
                .unwrap_or(&0.0);

            self.input_item(resource_value, input_limit, parent_index, graph)
        } else if let Some(provided_input) =
            self.config.find_input_by_resource(resource_value.resource)
        {
            self.input_item(resource_value, provided_input.value, parent_index, graph)
        } else {
            self.produce_item(resource_value, parent_index, graph)
        }
    }
        ...
        if let Some(existing_node_index) = self.find_input_node(graph, resource_value.resource) {
            match &mut graph[existing_node_index] {
                PlanGraphNode::InputNode(existing_resource_value) => {
                    if existing_resource_value.value + resource_value.value > input_limit {
                        if resource_value.resource.is_raw() {
                            return Err(SolverError::MissingInput(resource_value.resource));
                        } else {
                            existing_resource_value.value = input_limit;
                            let remaining_input = ResourceValuePair::new(
                                resource_value.resource,
                                resource_value.value - input_limit,
                            );
            ...
            let (input_value, remaining_input) = if resource_value.value > input_limit {
                (
                    ResourceValuePair::new(resource_value.resource, input_limit),
                    Some(ResourceValuePair::new(
                        resource_value.resource,
                        resource_value.value - input_limit,
                    )),
                )
            ...
            remaining_input.map_or(Ok(()), |rv| self.produce_item(rv, parent_index, graph))?;
        }
    ...
    fn propagate_production_changes(
        ...
        additional_output: ResourceValuePair<f64>,
        graph: &mut GraphType<'a>,
    ) -> SolverResult<()> {
        let additional_inputs: Vec<ResourceValuePair<f64>> = match &mut graph[node_index] {
            PlanGraphNode::ProductionNode(existing_recipe, existing_machine_count) => {
                let output = existing_recipe
                    .find_output_by_item(additional_output.resource)
                    .unwrap();
    ...
    fn find_input_node(&self, graph: &GraphType<'a>, resource: Resource) -> Option<NodeIndex> {
        graph
            .node_indices()
            .find(|i| graph[*i].is_input_for_item(resource))
    }
